...
std::vector<PropertyMap> EntityManagerInterface::queryAllDevices()
{
    ...
    catch (const std::exception& e)
    {
        ...
        lg2::warning("EntityManager not available or query failed: {ERROR}",
                     "ERROR", e.what());
        lg2::warning(
...
void EntityManagerInterface::handleInterfacesAdded(sdbusplus::message_t& msg)
{
    ...
    {
        lg2::error("Failed to read InterfacesAdded signal: {ERROR}", "ERROR",
                   e.what());
        return;
    }
    ...
    if (deviceAddedCallback)
    {
        ...
        {
            lg2::error("Device added callback failed: {ERROR}", "ERROR",
                       e.what());
        }
    }
}
